...
import { Minus, Plus, Sun, Moon, Lamp } from 'lucide-react';
...
export const SettingsPanel = memo(function SettingsPanel({
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed bottom-0 left-0 right-0 bg-card border-t border-border rounded-t-2xl p-6 z-50 transition-transform duration-300 ${
      ...
      >
        <div className="max-w-md mx-auto space-y-6">
            ...
            <div className="flex items-center gap-2">
              {[1.4, 1.6, 1.8, 2.0].map((lh) => (
                <button
                  ...
                  className={`px-3 py-1 text-xs font-serif rounded-full transition-colors ${
            ...
            <div className="flex items-center gap-2">
              {(['narrow', 'normal', 'wide'] as const).map((m) => (
                <button
                  ...
                  className={`px-3 py-1 text-xs font-serif capitalize rounded-full transition-colors ${
                    settings.margins === m
                      ? 'bg-primary text-primary-foreground'
                      : 'bg-secondary text-secondary-foreground hover:bg-accent'
                  }`}
          ...
          {/* Theme */}
          <div className="flex items-center justify-between">
            <span className="text-sm text-muted-foreground font-serif">Theme</span>
            <div className="flex items-center gap-2">
              <button
                type="button"
